...
"""Execution of the LF_WR function
Properties:
    Method: 1 - Fact table load
    Target table: web_returns
"""

def create_view(connection):
    print('\tCreating view wrv')
    query = '''CREATE VIEW wrv AS
SELECT
...
FROM
	...
	LEFT OUTER JOIN time_dim ON (
		(
			CAST(SUBSTR(wret_return_time, 1, 2) AS integer) * 3600 + CAST(SUBSTR(wret_return_time, 4, 2) AS integer) * 60 + CAST(SUBSTR(wret_return_time, 7, 2) AS integer)
		) = t_time
	)
...
def insert_from_view(connection):
    query = 'INSERT OR IGNORE INTO web_returns SELECT DISTINCT(*) FROM wrv'
    print(f'\tQuery: {query}')
    connection.sql(query)

def execute(scale, test, run, connection):
    stream = (test - 1) * 2 + (run - 1) + 1
    print(f'\nExecuting function LF_WR, test #{test}, run #{run}, stream #{stream}')
    start_time = time()
