...
"""Execution of the LF_CS function
Properties:
    Method: 1 - Fact table load
    Target table: catalog_sales
"""

def create_view(connection):
    print('\tCreating view csv')
    query = '''CREATE view csv as
SELECT
...
FROM
	...
	LEFT OUTER JOIN call_center ON (
...
def insert_from_view(connection):
    query = 'INSERT OR IGNORE INTO catalog_sales SELECT DISTINCT(*) FROM csv'
    print(f'\tQuery: {query}')
    connection.sql(query)

def execute(scale, test, run, connection):
    stream = (test - 1) * 2 + (run - 1) + 1
    print(f'\nExecuting function LF_CS, test #{test}, run #{run}, stream #{stream}')
    start_time = time()
