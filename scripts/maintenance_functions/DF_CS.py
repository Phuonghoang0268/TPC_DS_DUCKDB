...
def execute(scale, test, run, connection):
    ...
    with open(flatfile, 'r') as file:
        for line in file:
            ...
            query_1 = f'DELETE FROM catalog_returns WHERE cr_order_number IN (SELECT cs_order_number FROM catalog_sales WHERE cs_sold_date_sk IN (SELECT d_date_sk FROM date_dim WHERE d_date BETWEEN {date_1} AND {date_2}));'
            query_2 = f'DELETE FROM catalog_sales WHERE cs_sold_date_sk IN (SELECT d_date_sk FROM date_dim WHERE d_date BETWEEN {date_1} AND {date_2});'
