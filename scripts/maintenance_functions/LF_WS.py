...
"""Execution of the LF_WS function
Properties:
    Method: 1 - Fact table load
    Target table: web_sales
"""

def create_view(connection):
    print('\tCreating view wsv')
    query = '''CREATE VIEW wsv AS
SELECT
	...
	(
		(wlin_sales_price * wlin_quantity) - wlin_coupon_amt
	) -(i_wholesale_cost * wlin_quantity) WS_NET_PROFIT
FROM
	s_web_order
...
def insert_from_view(connection):
    query = 'INSERT OR IGNORE INTO web_sales SELECT DISTINCT(*) FROM wsv'
    print(f'\tQuery: {query}')
    connection.sql(query)

def execute(scale, test, run, connection):
    stream = (test - 1) * 2 + (run - 1) + 1
    print(f'\nExecuting function LF_WS, test #{test}, run #{run}, stream #{stream}')
    start_time = time()
