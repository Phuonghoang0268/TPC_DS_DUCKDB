...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 120212
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 988
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 100078
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 134932
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1998) and
        ((i_category in ('Children','Men','Electronics') and
          i_class in ('school-uniforms','pants','portable')
         )
      or (i_category in ('Shoes','Sports','Jewelry') and
          i_class in ('kids','basketball','diamonds') 
        ))
...
where 
	ws_item_sk = i_item_sk 
  	...
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('2001-01-18' as date) 
				and (cast('2001-01-18' as date) + interval  '30 days')
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast('2001-05-02' as date) 
...
select  i_item_id
 ...
 and i_manufact_id in (791,463,179,908)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2001-08-02' as date) 
                  and (cast('2001-08-02' as date) +  interval '30 days') 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Riverside'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  39446
   and ib_upper_bound   <=  39446 + 50000
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('ghost','maroon','sky'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 2
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (6,2,12,7,9,8) and
       d_year = 2002 and
       ca_state in ('ND','NE','NJ'
                   ,'OK','MI','OH','LA')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-03-31','1999-09-01','1999-11-03')))
...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('2002-08-23' as date) 
                  and (cast('2002-08-23' as date) +  interval '14 days')
