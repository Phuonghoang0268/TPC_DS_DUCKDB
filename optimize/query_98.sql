WITH date_range AS (
  SELECT d_date_sk
  FROM date_dim
  WHERE d_date BETWEEN CAST('2001-08-04' AS DATE)
...
store_data AS (
  ...
  FROM (
    ...
    FROM store_sales
    WHERE ss_sold_date_sk IN (SELECT * FROM date_range)
    UNION ALL
    SELECT
    ...
    FROM store_returns
    WHERE sr_returned_date_sk IN (SELECT * FROM date_range)
  ) salesreturns
...
catalog_data AS (
  ...
  FROM (
    SELECT
      ...
      CAST(0 AS DECIMAL(7,2)) AS net_loss
    FROM catalog_sales
    WHERE cs_sold_date_sk IN (SELECT * FROM date_range)
    UNION ALL
    SELECT
    ...
    WHERE cr_returned_date_sk IN (SELECT * FROM date_range)
  ) salesreturns
...
web_data AS (
  ...
  FROM (
    SELECT
      ...
      CAST(0 AS DECIMAL(7,2)) AS net_loss
    FROM web_sales
    WHERE ws_sold_date_sk IN (SELECT * FROM date_range)
    UNION ALL
    SELECT
      ws_web_site_sk AS wsr_web_site_sk,
    ...
    WHERE wr_returned_date_sk IN (SELECT * FROM date_range)
  ) salesreturns
